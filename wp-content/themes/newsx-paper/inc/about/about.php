...
{
	global $pagenow;
	if (get_option('newsxpaper2') || $pagenow == 'themes.php') {
		return;
	}
	$class = 'eye-notice notice notice-warning is-dismissible';
	$message = __('<strong>Wow!! Amazing offer!! NewX Paper Pro Theme 30% Early Bird Offer!!! Use coupon ( nxp30erb )!!<br /> Only active the Pro theme and follow auto-setup then your site will be ready with lots of advanced features. Now create your site with the pro with only a few clicks!!!! So why late get pro now.</strong> ', 'newsx-paper');
	$url1 = esc_url('https://news.wpteamx.com/newsx-paper-pro-demos/');
	$url3 = esc_url('https://wpthemespace.com/product/newsx-paper-pro/?add-to-cart=6534');

	printf('<div class="%1$s" style="padding:10px 15px 20px;text-transform:uppercase"><p>%2$s</p><a target="_blank" class="button button-primary" href="%3$s" style="margin-right:10px">' . __('NewsX Paper Pro Details', 'newsx-paper') . '</a><a target="_blank" class="button button-primary" href="%4$s" style="margin-right:10px">' . __('Upgrade Pro', 'newsx-paper') . '</a><button class="button button-info btnend" style="margin-left:10px">' . __('Dismiss the notice', 'newsx-paper') . '</button></div>', esc_attr($class), wp_kses_post($message), $url1, $url3);
}
...
function newsx_paper_new_optins_texts_init()
{
	if (isset($_GET['xbnotice']) && $_GET['xbnotice'] == 1) {
		delete_option('newsxpaper1');
		update_option('newsxpaper2', 1);
	}
}
