 ...
 * @since 1.0.0
 */
if (!class_exists('newsx_paper_About')) {
	/**
	 * Singleton class used for generating the about page of the theme.
	 */
	class newsx_paper_About
	{
		/**
		 ...
		 *
		 * @var newsx_paper_About $instance The  TI_About_Page instance.
		 */
		...
		{
			if (!isset(self::$instance) && !(self::$instance instanceof newsx_paper_About)) {
				self::$instance = new newsx_paper_About;
		...
		public function ti_about_page_render()
		{
			...
			if (!empty($welcome_title) || !empty($welcome_content) || !empty($this->tabs)) {
				...
				echo '<div class="wrap about-wrap newsx-about-wrap">';
				...
				echo '<a href="' . esc_url('https://news.wpteamx.com/newsx-paper-pro-demos/') . '" target="_blank" class="wp-badge epsilon-welcome-logo"></a>';
...
?>
			<div class="feature-section newsx-section newsx-section-getting-started three-col">
				<?php foreach ($content as $item) : ?>
					<?php $this->render_grid_item($item); ?>
				<?php endforeach; ?>
			</div><!-- .feature-section .newsx-section -->
		<?php
		}
		...
		?>
			<div class="feature-section newsx-section newsx-section-support three-col">
		...
		public function free_pro()
		{
			...
			if (!empty($free_pro)) {
				if (!empty($free_pro['free_theme_name']) && !empty($free_pro['pro_theme_name']) && !empty($free_pro['features']) && is_array($free_pro['features'])) {
					...
					echo '<tr><td style="border-top:0;"></td><td style="border-top:0;"></td><td style="border-top:0;" class="only-pro"><a href="' . esc_url('https://news.wpteamx.com/newsx-paper-pro-demos/') . '" target="_blank" >';
					echo esc_html('NewsX Paper Pro Live Preview', 'newsx-paper');
					echo '</a></td></tr>';
			...
			if ('appearance_page_' . $this->theme_slug . '-welcome' == $hook_suffix) {
				...
				wp_localize_script(
					...
					array(
						...
						'activating_string'        => __('Activating', 'newsx-paper')
					)
				);
			}
		}
