 ...
 * @return array
 */
function bakerystore_body_classes( $classes ) {
...
add_filter( 'body_class', 'bakerystore_body_classes' );
...
if (!function_exists('bakerystore_str_replace_assoc')) {

    /**
     * bakerystore_str_replace_assoc
     * @param  array $replace
     * @param  array $subject
     * @return array
     */
    function bakerystore_str_replace_assoc(array $replace, $subject) {
 ...
 */
function bakerystore_add_to_cart_fragment( $fragments ) {
	...
	<a class="nav-link cart-icon-wrap header-cart" href="javascript:void(0);" role="button" data-bs-toggle="collapse" data-bs-target="#Cart">
		...
		?>	
			<span><?php echo esc_html__('0','bakery-store'); ?></span>
		<?php
		}
		?>
	</a>
...
add_filter( 'woocommerce_add_to_cart_fragments', 'bakerystore_add_to_cart_fragment' );
...
if ( ! function_exists( 'bakerystore_breadcrumb_title' ) ) {
	function bakerystore_breadcrumb_title() {
		...
		elseif ( is_day() ) : 
			...
			printf( __( 'Daily Archives: %s', 'bakery-store' ), get_the_date() );
		
		elseif ( is_month() ) :
		
			printf( __( 'Monthly Archives: %s', 'bakery-store' ), (get_the_date( 'F Y' ) ));
			
		elseif ( is_year() ) :
		
			printf( __( 'Yearly Archives: %s', 'bakery-store' ), (get_the_date( 'Y' ) ) );
			
		elseif ( is_category() ) :
		
			printf( __( 'Category Archives: %s', 'bakery-store' ), (single_cat_title( '', false ) ));

		elseif ( is_tag() ) :
		
			printf( __( 'Tag Archives: %s', 'bakery-store' ), (single_tag_title( '', false ) ));
			
		elseif ( is_404() ) :

			printf( __( 'Error 404', 'bakery-store' ));
			
		elseif ( is_author() ) :
		
			printf( __( 'Author: %s', 'bakery-store' ), (get_the_author( '', false ) ));		
...
function bakerystore_breadcrumbs() {
	
	$showOnHome	= esc_html__('1','bakery-store'); 	// 1 - Show breadcrumbs on the homepage, 0 - don't show
	$delimiter 	= '';   // Delimiter between breadcrumb
	$home 		= esc_html__('Home','bakery-store'); 	// Text for the 'Home' link
	$showCurrent= esc_html__('1','bakery-store'); // Current post/page title in breadcrumb in use 1, Use 0 for don't show
    ...
    if ( is_category() ) 
	{
		...
		echo $before . esc_html__('Archive by category','bakery-store').' "' . esc_html(single_cat_title('', false)) . '"' .$after;
		
	} 
	
	elseif ( is_search() ) 
	{
		echo $before . esc_html__('Search results for','bakery-store').' "' . esc_html(get_search_query()) . '"' . $after;
	} 
	...
	elseif ( is_tag() ) 
	{
		echo $before . esc_html__('Posts tagged ','bakery-store').' "' . esc_html(single_tag_title('', false)) . '"' . $after;
	} 
	
	elseif ( is_author() ) {
		global $author;
		$userdata = get_userdata($author);
		echo $before . esc_html__('Articles posted by ','bakery-store').'' . $userdata->display_name . $after;
	} 
	
	elseif ( is_404() ) {
		echo $before . esc_html__('Error 404 ','bakery-store'). $after;
    }
	
    if ( get_query_var('paged') ) {
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
		echo ' ( ' . esc_html__('Page','bakery-store') . '' . esc_html(get_query_var('paged')). ' )';
		if ( is_category() || is_day() || is_month() || is_year() || is_search() || is_tag() || is_author() ) echo '';
    }
...
add_action( 'wp_ajax_bakerystore_dismissed_notice_handler', 'bakerystore_ajax_notice_handler' );

/**
 * AJAX handler to store the state of dismissible notices.
 */
function bakerystore_ajax_notice_handler() {
    if ( isset( $_POST['type'] ) ) {
...
add_action( 'wp_ajax_install_act_plugin', 'bakerystore_admin_install_plugin' );

function bakerystore_admin_install_plugin() {
    /**
     * Install Plugin.
     */
